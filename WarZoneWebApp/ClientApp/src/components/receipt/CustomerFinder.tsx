...
const CustomerFinder: FunctionComponent<ICustomerFinderProps> = (props: ICustomerFinderProps) => {
  ...
  const searchResults = () => {
    return rawSearchData
      ? rawSearchData.map((searchResult) => (
          <Select.Option
            value={`${searchResult.customerName!} ${searchResult.customerSurname!}`}
            key={
              searchResult.id
            }>{`${searchResult.customerName!} ${searchResult.customerSurname!}`}</Select.Option>
        ))
      : undefined;
  };
  ...
  const onDeselect = (customerName: string) => {
    const customer = props.customers.find(
      (customer) => `${customer.customerName} ${customer.customerSurname}` === customerName
    );
    if (customer) {
      var filtered = props.customers.filter((value) => {
    ...
    <>
      <Form
        ...
        onFieldsChange={(changedFields, allFields) => {
          ...
          setWaitingTimeout(
            setTimeout(() => onCustomerNameFieldChange(changedFields[0].value as string), 200)
          );
        }}>
        <Form.Item
          style={{ display: "block" }}
