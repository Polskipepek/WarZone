      ...
      <Layout className="layout">
        <Header color="white">
          <div className="logo" onClick={() => changePage(Resources.pageAdresses.home)}>
            <img
              src="http://www.war-zone.com.pl/wp-content/uploads/2016/01/LOGO_FB.png"
              style={{
                maxHeight: "48px",
                height: "auto",
                width: "auto",
          ...
          <Menu
            ...
            {appUser && (
              <Menu.Item onClick={() => changePage(Resources.pageAdresses.consent)} key="2">
                Regulamin
              </Menu.Item>
            )}
            <Menu.Item onClick={() => changePage(Resources.pageAdresses.offer)} key="3">
              Oferta
            </Menu.Item>
            {appUser && appUser.id == 1 && (
              <Menu.Item onClick={() => changePage(Resources.pageAdresses.receipts)} key="4">
                Otwarte rachunki
              </Menu.Item>
            )}
            {appUser && appUser.id == 1 && (
              <MenuItem
                ...
                style={{ float: "right" }}
                onClick={() => logout()}>
                <LogoutOutlined style={{ verticalAlign: "baseline" }} />
                Wyloguj
              </MenuItem>
            )}
          </Menu>
        ...
        <Content style={{ padding: "80px 25px" }}>
          <div
            style={{ background: "#fff", padding: 24, minHeight: window.screen.availHeight * 0.9 }}>
            {props.children}
          </div>
        </Content>
        <Footer style={{ textAlign: "center" }}>
          <div className="kliven-centered">Created by KlivenINC</div>
        </Footer>
      </Layout>
    </>
  );
