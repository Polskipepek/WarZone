...
import { ReceiptClient, ReceiptAndCustomerBinderClient } from "../../ApiClient";
...
const InnerConsentForm: FunctionComponent<IInnerConsentForm> = (props) => {
  ...
  const submitForm = (name: string, surname: string) => {
    ...
    new ReceiptAndCustomerBinderClient().addAvailableCustomers(name, surname).catch((ex) => {
      openErrorNotification(`Błąd przy tworzeniu klienta.`, ``);
    });
      ...
      <div className="kliven-centered">
        <Form
          ...
          <Form.Item
            ...
            <Input size="large" type="text" placeholder={"Imię"} />
          </Form.Item>
          <Form.Item
            name="surname"
            label="Nazwisko"
            ...
            <Input size="large" type="text" placeholder={"Nazwisko"} />
          </Form.Item>
          ...
          <Form.Item
            ...
            rules={[
              {
                validator: (_, value) =>
                  value ? Promise.resolve() : Promise.reject("Wymagana zgoda"),
              },
            ]}
            {...tailLayout}>
            <Checkbox>
              Akceptuję{" "}
              <Link to="#" onClick={() => onConsentClick()}>
                regulamin
              </Link>
            </Checkbox>
          </Form.Item>
